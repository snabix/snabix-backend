...
Всего PHP строк по `app`: около `24 800`.
...
- [x] [P1] Разбить `src/features/listing/ui/listing-form.tsx`.
  ...
  - Выполнено 2026-06-22: `listing-form.tsx` сокращён до точки входа, create/edit layouts разделены, а категории, характеристики, цена, адрес, медиа и submit bar вынесены в самостоятельные UI-компоненты. Состояние и orchestration сохранены в `useListingFormState`; integration-тесты формы подтверждают прежние create/edit сценарии.

- [x] [P1] Разбить `src/features/listing/model/use-listing-form-state.ts`.
  ...
  - Предложение: выделить хуки:
    ...
    - `useListingSubmit`
  - Выполнено 2026-06-22: category tree, attribute values, address loading, media lifecycle и submit flow вынесены в пять специализированных hooks. `useListingFormState` сокращён до явной композиции зависимостей и сохраняет прежний UI-контракт. Integration-тест дополнен успешным retry загрузки медиа.
...
- [x] [P1] Убрать дублирование `ListingPayloadMapper` и `PublicListingPayloadMapper`.
  ...
  - Выполнено 2026-06-21: общая сборка category/breadcrumbs, location, image media и attribute values перенесена в `ListingPayloadAssembler`. Оба endpoint-specific mapper сохранены как тонкие адаптеры режимов `PRIVATE_VIEW`/`PUBLIC_VIEW`. `ListingPayloadVisibilityPolicy` применяет явный public allowlist и отдельно допускает только attribute values с `show_in_card`; неизвестные будущие поля по умолчанию не публикуются. Private-only поля и подробный `media` закреплены API DTO документацией и regression tests. PHPStan, Scramble и 119 backend tests проходят.
...
- [x] [P1] Разделить `ListingInputNormalizer`.
  ...
  - Выполнено 2026-06-21: монолитный `ListingInputNormalizer` удалён. Добавлены сценарные `ListingCreateNormalizer`, `ListingUpdateNormalizer`, `ListingModerationNormalizer`, общий normalizer классификации и allowlist пользовательских полей. Цена, валюта и контакты вынесены в domain value objects. Create/update handlers выбирают normalizer до обращения к writer; `EloquentListingWriter` принимает готовый `NormalizedListingData` и больше не интерпретирует сырой payload. Update normalizer физически не может сформировать moderation-поля, а moderation normalizer централизует безопасные defaults и status transitions. Добавлены unit/feature boundary tests; PHPStan, Scramble и 116 backend tests проходят.
...
- [x] [P1] Breadcrumbs лучше отдавать из backend как готовую структуру без дополнительных запросов в listing mapper.
  - Выполнено 2026-06-21 и повторно проверено 2026-06-22: `CategoryBreadcrumbService` формирует готовые `breadcrumbs/fullName`, кэширует category index и resolved trails на время запроса. Query-test подтверждает один общий category lookup для коллекции и отсутствие запросов при повторном mapping.
- [x] [P2] Frontend category store стоит покрыть тестами cache invalidation и повторной загрузки веток.
  - Выполнено 2026-06-22: тесты покрывают cache reuse, root/branch/attribute invalidation, forced branch reload, истечение TTL и независимое обновление соседних веток.
...
- [x] [P2] `src/screens/blog/model/posts.ts` содержит большой локальный контент/модель. Нужно отделить static fallback от runtime API adapter.
  - Выполнено 2026-06-21: монолитный `posts.ts` удалён. Типы, hero slides и fallback posts разделены на независимые model-модули; fallback-файл укладывается в лимит 300 строк. Runtime API вынесен в `news-runtime-source`, а единый `load-blog-content` централизует fallback для списка и detail. App Router страницы больше не смешивают API adapter и локальные данные. Добавлены тесты runtime/empty/error/not-found сценариев.
- [x] [P2] Для изображений в blog стоит заменить raw `<img>` там, где это не blob preview.
  - Выполнено 2026-06-20 и повторно проверено 2026-06-21: blog hero, cards, list и content blocks используют shared `MediaImage` поверх `next/image`; raw `<img>` и `@next/next/no-img-element` suppressions в blog отсутствуют. Поведение `MediaImage` покрыто regression tests.
...
- [x] Разбить `listing-form.tsx`. Выполнено 2026-06-22.
- [x] Разбить `use-listing-form-state.ts`. Выполнено 2026-06-22.
...
- [x] Разделить `ListingInputNormalizer`. Выполнено 2026-06-21.
- [x] Объединить public/private listing payload mapping. Выполнено 2026-06-21.
