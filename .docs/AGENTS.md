...
`Snabix Backend` построен на `Laravel 12`, `PHP 8.3`, `Sanctum`, `Filament 5`, `Scramble` и health-check пакете.
...
- Бизнес-логика должна жить в `Application/UseCases` и доменных сервисах, а не в контроллере или Eloquent-репозитории.
- Репозиторий отвечает за persistence, выборки и нормализацию хранения. Правила публикации, черновиков и переходов состояния постепенно выноси в `Domain/Services` или application service.
- Административные API после подключения ролей должны проверять не только guard, но и `Gate` / policy permission там, где действие меняет данные.
...
- убедись, что endpoint корректно попадает в `Scramble`-документацию;
...
- Роли и permissions админки работают через `filament-shield` и `spatie/laravel-permission`.
- Admin model: `App\Auth\Infrastructure\Models\EloquentAdmin`, guard: `admin`.
- Для новых Filament resources добавляй policy/permissions и проверяй сценарий обычного admin без прав.
...
- `Scramble` analysis не сломан;
- feature-тесты на новый API написаны;
- если API возвращает список, frontend-контракт `items/meta` синхронизирован и покрыт тестами;
